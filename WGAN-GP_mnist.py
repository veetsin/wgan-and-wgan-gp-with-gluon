...
logging.basicConfig(level=logging.DEBUG,filename='WGAN-gp_mnist.log',filemode='w',format='[%(levelname)s:%(message)s]')
#his_acc_real = []
#his_acc_fake = []
his_wassD = []
his_errG = []
...
path_D = 'his_params_D_WGAN-gp_20181023001'
path_G = 'his_params_G_WGAN-gp_20181023001'
...
for epoch in range(epochs):
    ...
    for batch in train_data:
        ...
        #========G fixed , train D,maxmize -D(x) + D(G(z))======
            ...
            fake = netG(latent_z)
#            x_hat_1 = []
            epsilon = nd.random.uniform(0,1,shape=[batch_size],ctx=ctx)
            x_hat = (epsilon*(data.reshape([batch_size,33*33]).T)).T.reshape([batch_size,1,33,33]) + ((1-epsilon)*(fake.reshape([batch_size,33*33]).T)).T.reshape([batch_size,1,33,33])
#            for i in range(batch_size):
#                epsilon = nd.random.uniform(0,1,ctx=ctx)
#                tem_xhat = epsilon*data[i] + (1-epsilon)*fake[i]
#                tem_xhat = tem_xhat.reshape((1,)+tem_xhat.shape)
#                x_hat.append(tem_xhat)
#            x_hat = nd.concatenate(x_hat)
...
#                metric_real.update(errD_real,real_label)
                ...
                tem_gp = nd.abs((netD(x_hat+e)-netD(x_hat-e))/(2*e))
                errD = errD_fake.mean() - errD_real.mean() + sum(lamda*nd.power(tem_gp-1,2))/batch_size
                extra_term = sum(lamda*nd.power(tem_gp-1,2))/batch_size
                errD = errD.mean()
                errD.backward()
...
#    his_errG.append(sum(sum_errG)/len(sum_errG))
    ...
    logging.info('epoch: %i ; Wasserstein distance(gp) :%f  ; time:%f '
                 %(epoch , (np.mean(sum_wassD)) ,end_time-start_time))
#    metric_real.reset()
#    metric_fake.reset()
...
plt.plot(x_axis,his_wassD,label='wasserstein distance(gp)')
plt.xlabel('epoch')
plt.legend()
plt.show()
